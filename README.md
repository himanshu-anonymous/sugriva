...
*   **Post-Quantum Agility:** Agility wrappers simulating NIST ML-KEM-768 key encapsulation and ML-DSA signature routines. Resolves to a fallback simulation header output when local cryptography bindings do not natively support finalized NIST algorithms.
...
### 5. Orchestrator and Worker Loop (`main.py`)
...
### 6. Interactive Curses Dashboard Terminal UI (`tools/sugriva_terminal_ui.py`)
...
*   **Analyst Command Console (Bottom):** Allows running commands to interact with the system on the fly (see Commands below).
...
Install all requirements. Note that `windows-curses` will automatically install on Windows platforms to support the terminal dashboard UI:
```bash
pip install -r requirements.txt
```

#### Step D: Seed the Persistence Layers
Run the startup seeder to initialize the SQLite schema and populate it with synthetic transactional history:
```bash
python run_mvp.py
```

#### Step E: Start the Orchestration Core
Launch the main FastAPI server and background worker loop:
```bash
python main.py
```
...
```
> **Note:** The dashboard requires a minimum terminal window size of **90x20**. If your terminal is too small, a friendly warning will display. Simply expand or maximize your terminal window to render the full layout.

#### Step G: Start the Stream Traffic Simulator (Optional)
...
| `inject <vpa> <amount> <ip> [SUCCESS/FAILED]` | Spoofs/injects a custom transaction into the telemetry pipeline | `inject attacker@bank 850000 10.0.0.1 FAILED` |
