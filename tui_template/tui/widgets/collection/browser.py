...
from tui.models import Collection, RequestModel
# from tui.config import SETTINGS
from tui.files import get_unique_request_filename
from tui.help_data import HelpData
from tui.save_request import generate_request_filename
from tui.widgets.collection.new_request_modal import (
    NewRequestData,
    NewRequestModal,
)
from tui.widgets.confirmation import ConfirmationModal
from tui.widgets.tree import PostingTree
...
""",
    ...
    async def new_request_flow(self, templated_from: RequestModel | None) -> None:
        ...
        def _handle_new_request_data(new_request_data: NewRequestData | None) -> None:
            ...
            self.app.backend.save_request(new_request, save_path)
    ...
    def action_quick_duplicate_request(self) -> None:
        ...
        self.app.backend.save_request(cursor_request_copy, new_path)
        self.add_request(
    ...
    def action_delete_request(self) -> None:
        ...
        if isinstance(cursor_node.data, RequestModel):
            cursor_request = cursor_node.data
            self.app.backend.delete_request(cursor_request)
            cursor_node.remove()
...
class CollectionBrowser(Vertical):
    ...
    def compose(self) -> ComposeResult:
        self.styles.dock = self.app.settings.collection_browser.position
