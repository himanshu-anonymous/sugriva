...
class QuantumGuardPane(Vertical):
    ...
    def _tick_quantum(self) -> None:
        ...
        elif event_type == "QKD_SYNC":
            coherence = random.uniform(98.5, 99.9)
            self._log.write(f"[{ts}] [bold yellow]QKD CHANNEL[/bold yellow] Entangled fiber state: [bold]{coherence:.2f}% coherence[/bold].")
        elif event_type == "ENTROPY_POLL":
            bits = random.choice([2048, 4096, 8192])
            self._log.write(f"[{ts}] [bold magenta]TRNG POOL[/bold magenta] Quantum Entropy: {bits} bits cached [green]OK[/green]")
