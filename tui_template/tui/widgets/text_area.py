...
from tui.exit_codes import GENERAL_ERROR
...
class PostingTextArea(TextArea):
    ...
    def on_mount(self) -> None:
        self.indent_width = 2
        self.cursor_blink = self.app.settings.text_input.blinking_cursor
    ...
    def action_open_in_editor(self) -> None:
        editor_command = self.app.settings.editor
...
class ReadOnlyTextArea(PostingTextArea):
    ...
    def watch_visual_mode(self, value: bool) -> None:
        self.cursor_blink = self.app.settings.text_input.blinking_cursor and not value
        if not value:
