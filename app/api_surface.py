...
app = FastAPI(title="Project Sugriva API Surface", lifespan=lifespan)
        ...
        for tx in transactions:
            if "xai_attributions" not in tx:
                try:
                    ...
                    features = [float(ip_val), auth_val, amt_val, 0.0]
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app.api_surface:app", host="0.0.0.0", port=int(os.getenv("SYSTEM_PORT", 8000)), reload=False)
